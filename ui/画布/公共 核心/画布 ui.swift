...
struct CanvasView: View {
    ...
    func startVideoGeneration(nodeID: UUID, prompt: String, visited: Set<UUID> = []) {
        ...
        debugLog("视频生成：入队（节点 \(nodeID)，提示词「\(trimmedPrompt)」，尺寸 \(size.width)×\(size.height)，时长 \(duration.displayName)）")
        let task = QueueTask(
            kind: .video,
            nodeID: nodeID,
    ...
    func attachGeneratedVideo(to nodeID: UUID, mp4Path: String, prompt: String) {
        ...
        while fm.fileExists(atPath: assetLibraryURL.appendingPathComponent("\(baseName).mp4").path) {
            baseName += "_"
        }
