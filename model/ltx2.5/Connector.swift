...
final class ConnectorBody: Module {
    ...
    func callAsFunction(_ input: MLXArray, nValid: Int) -> MLXArray {
        let T = input.shape[1]
        let dim = self.dim
        let inputBf = input.asType(.bfloat16)

        // register 替换左 pad：valid（右 nValid 个）在前，registers 填充剩余
        let reg3 = learnable_registers.asType(.bfloat16).reshaped([1, numRegisters, dim])
        var tiles = [MLXArray]()
        ...
        let (cosF, sinF) = connectorRopeFreqs(T: T, dim: dim, headDim: headDim)
        let cosB = cosF.asType(.bfloat16)
        let sinB = sinF.asType(.bfloat16)
        var h = x
