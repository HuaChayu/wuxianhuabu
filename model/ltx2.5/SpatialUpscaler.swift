...
func upResBlock(
    ...
    let residual = x
    let c1 = upscalerConv3d(x, weight: weights[base + ".conv1.weight"]!, bias: weights[base + ".conv1.bias"])
    let n1 = groupNormNDHWC(c1, weight: weights[base + ".norm1.weight"]!, bias: weights[base + ".norm1.bias"]!)
    let a1 = silu(n1)
    let c2 = upscalerConv3d(a1, weight: weights[base + ".conv2.weight"]!, bias: weights[base + ".conv2.bias"])
...
func latentUpsamplerSpatial(
    ...
    var x = upscalerConv3d(latentNDHWC,
