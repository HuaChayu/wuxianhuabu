...
func hidreamBuildT2ISample(
    ...
    let mask4d = hidreamBuildAttentionMask(tokenTypesBin: tokenTypes.map { $0 > 0 })
...
func hidreamBuildAttentionMask(tokenTypesBin: [Bool]) -> MLXArray {
    ...
    // gen 行（token_types>0）全部置 0 → 双向可见
...
/// 跨次生成（同 shape）不重编译。
/// 通用 CompiledForwardCache<Owner,F> 封装 CompiledGraphCache 多槽 LRU，
/// 模型无关（见 model/通信&调用/优化方法.swift），T2I 与编辑用 shapeKey 前缀天然隔离。
private let hiDreamCompiledForward = CompiledForwardCache<HiDreamBackbone, CompiledHiDreamForward>()
...
func hidreamGenerate(
    ...
    // MLX.compile 单步前向：shape 固定则复用编译图（对齐优化方法.swift 通用 CompiledForwardCache）
    let shapeKey =
    ...
    if mlxCompileEnabled {
        forward = hiDreamCompiledForward.resolve(owner: backbone, key: shapeKey) {
    ...
    for (i, stepT) in scheduler.timestepsNP.enumerated() {
        ...
        let xPred: MLXArray
        if let rv = teaCache.next(sigma: sigma) {
            xPred = rv
            print("    [TeaCache] 步 \(i + 1) σ=\(sigma) 命中缓存，跳过前向")
        } else {
            // 峰值保底：真实重前向前按任务尺寸估算 projected 做保底（对齐 LTX Sampler.swift L112），
            // TeaCache 命中步直出自缓存不经此处，天然不接入。
            _ = SystemMemory.ensureLoose(projected: Int64(MemoryPolicy.safetyBuffer(
                for: .image, width: w, height: h, frames: 0)))
            if let f = forward {
...
// MARK: - 参考图加载与预处理（py resize_pilimage / patchify_ref_image 移植）

/// 加载图片并归一化到 RGB 颜色空间，返回 CGImage（供后续两次 resize 复用）。
func hidreamLoadCGImage(_ path: String) -> CGImage? {
    ...
    if cg.colorSpace?.model != .rgb {
        ...
        guard let nsImg = NSImage(data: pngData),
              let rgbImg = nsImg.cgImage(forProposedRect: nil, context: nil, hints: nil) else { return nil }
        cg = rgbImg
    }
    return cg
}

/// 等比缩放 + 居中裁剪到 (newW, newH)，返回 RGBA float（0-255，row0=顶部）。
/// 近似 py resize_pilimage 的 bicubic + center-crop（省略 box 多级降采样，效果等价于单次高质量缩放）。
func hidreamResizeCropRGBA(_ cg: CGImage, to newW: Int, newH: Int) -> [UInt8]? {
    let sw = cg.width, sh = cg.height
    let scale = max(Float(newW) / Float(sw), Float(newH) / Float(sh))
    let dstW = max(Int((Float(sw) * scale).rounded()), 1)
    let dstH = max(Int((Float(sh) * scale).rounded()), 1)
    let offX = (newW - dstW) / 2
    let offY = (newH - dstH) / 2
    var px = [UInt8](repeating: 0, count: newW * newH * 4)
    guard let ctx = CGContext(
        data: &px, width: newW, height: newH, bitsPerComponent: 8,
        bytesPerRow: newW * 4, space: CGColorSpaceCreateDeviceRGB(),
...
/// - Returns: (newW, newH)，均为 32 倍数且面积 ≤ imageSize²。
...
/// DBG 日志：同时写 stderr 与 temp 文件（自验证用，不参与业务逻辑）。
private let hidreamDbgPath = "/Users/huachayui/Library/Application Support/com.tencent.mac.marvis/MarvisData/User/99999343CE761436DB7BAC928475A2A3/workspace/conv_1a018c64719_a1d5c2ed1673/temp/hidream_edit_dbg.log"
private func dbgLog(_ s: String) {
    let data = (s + "\n").data(using: .utf8) ?? Data()
    FileHandle.standardError.write(data)
    if let fh = FileHandle(forWritingAtPath: hidreamDbgPath) {
        fh.seekToEndOfFile()
        fh.write(data)
        try? fh.close()
    }
