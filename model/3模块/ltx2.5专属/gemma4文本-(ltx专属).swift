...
/// 数学实现已归纳到 通用算子/算子-RoPE.swift 的 proportionalRopeFreqs，此处仅绑定 Gemma4 配置参数。
func gemma4GlobalRopeFreqs() -> MLXArray {
    proportionalRopeFreqs(
        headDim: Gemma4Config.globalHeadDim,
        partialRotaryFactor: Gemma4Config.partialRotaryFactor,
        theta: Gemma4Config.globalRopeTheta)
}
...
final class Gemma4TextEncoder: Module {
    ...
    func encodeStates(_ ids: MLXArray) -> [MLXArray] {
        ...
        let mask = causalPadMask(ids: ids, padTokenID: Gemma4Config.padTokenID)
    ...
    private static func applyPenalties(
        ...
        if repeatPenalty != 1.0 {
            let rp = MLXArray(repeatPenalty, dtype: current.dtype)
            let invRp = MLXArray(1.0 / repeatPenalty, dtype: current.dtype)
            let zero = MLXArray(0.0, dtype: current.dtype)
        ...
        if presencePenalty != 0.0 {
            let pp = MLXArray(presencePenalty, dtype: current.dtype)
            let subtract = mask.asType(current.dtype) * pp
